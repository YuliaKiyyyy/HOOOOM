...
def get_rarest_char(file_path: str) -> str:
    with open(file_path, encoding='utf-8', mode='r') as file:
        counter = Counter(file.read().encode('utf-8').decode('unicode-escape'))  # The rarest letter considering case
...
def get_most_common_non_ascii_char(file_path: str) -> str:
    with open(file_path, mode='r') as file:
        ...
        for i in text:
            if not i.isascii():
                ascii_simbols += i
        counter = Counter(ascii_simbols)  # The rarest letter considering case
